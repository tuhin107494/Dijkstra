...
ll n,sz,k,m,dis[100005],par[100005],cs=0;
vector< pair<ll,ll> >v[100005];
void fun()
{
    for(int i=1; i<100005; i++)dis[i]=mx18;
    ...
    while(q.size())
    {
        ...
        {
            if(dis[v[u][i].F]>dis[u]+(v[u][i].S))
            {

                dis[v[u][i].F]=dis[u]+(v[u][i].S);
                q.push({-dis[v[u][i].F],v[u][i].F});
                par[v[u][i].F]=u;
            }
